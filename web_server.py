...
app = FastAPI(title="Quant Store Terminal")

connected_sockets = set()
paper_positions = []
pos_counter = 1

# Spot prices with live default fallbacks
spot_prices = {"BTCUSDT": 65120.0, "ETHUSDT": 3480.0, "XAUTUSDT": 2385.0}
...
<body class="text-[#1d1d1f] h-screen p-5 flex items-center justify-center overflow-hidden select-none">
  <div class="w-full h-full max-w-6xl max-h-[92vh] appstore-window rounded-2xl flex overflow-hidden">
    ...
    <main class="flex-1 p-8 overflow-y-auto bg-white space-y-6">
      ...
      <div id="tab-discover" class="space-y-6">
        ...
        <div class="card-banner rounded-2xl p-7 text-white flex flex-col justify-between min-h-[240px] shadow-lg">
          ...
          <div class="flex items-center justify-between pt-4">
            <div class="text-xs font-medium text-pink-100">Live Binance Feed Active</div>
            <button onclick="switchTab('tab-gex')" class="px-5 py-2 rounded-full bg-white text-gray-900 font-bold text-xs shadow-md hover:bg-gray-100 transition">Open 3D Mesh</button>
          </div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div class="bg-[#f5f5f7] p-5 rounded-2xl flex flex-col justify-between space-y-4 hover:shadow-sm transition">
            <div>
              <div class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">0.24Δ Squeeze</div>
          ...
          <div class="bg-[#f5f5f7] p-5 rounded-2xl flex flex-col justify-between space-y-4 hover:shadow-sm transition">
            <div>
              <div class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">High-Beta Gamma</div>
          ...
          <div class="bg-[#f5f5f7] p-5 rounded-2xl flex flex-col justify-between space-y-4 hover:shadow-sm transition">
            <div>
              <div class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Spring Flow</div>
      ...
      <div id="tab-gex" class="hidden space-y-4">
        <h1 class="text-2xl font-bold text-gray-900">3D Net Gamma Exposure (GEX)</h1>
        <div class="bg-[#f5f5f7] p-4 rounded-2xl">
          <div id="plotly-3d" class="w-full h-[420px]"></div>
        </div>
      </div>

      <!-- TAB 3: LIVE POSITIONS -->
      <div id="tab-positions" class="hidden space-y-4">
        <h1 class="text-2xl font-bold text-gray-900">Active Positions Log</h1>
        <div class="bg-[#f5f5f7] rounded-2xl overflow-hidden">
          <table class="w-full text-left text-xs">
            <thead class="bg-[#ebebeb] text-gray-500 font-semibold border-b border-gray-200">
              <tr>
                ...
                <th class="py-3 px-4 text-right">PNL (INR)</th>
                <th class="py-3 px-4 text-center">HOLD</th>
              </tr>
            </thead>
  ...
  <script>
    ...
    ws.onmessage = (e) => {
      ...
      const surfaceTrace = {
        z: data.gex_3d.matrix,
        x: data.gex_3d.strikes,
        y: data.gex_3d.expirations,
        type: 'surface',
        colorscale: [[0.0, '#f43f5e'], [0.5, '#e0e7ff'], [1.0, '#007aff']],
        showscale: false
      };
      ...
      const layout = {
        ...
        scene: {
          xaxis: { title: 'Strike', tickfont: { color: '#6b7280', size: 8 } },
          yaxis: { title: 'DTE', tickfont: { color: '#6b7280', size: 8 } },
          zaxis: { title: 'Gamma', tickfont: { color: '#6b7280', size: 8 } }
        }
      };

      if (!plotlyInit) {
        Plotly.newPlot('plotly-3d', [surfaceTrace], layout, { responsive: true });
        plotlyInit = true;
      } else {
        Plotly.react('plotly-3d', [surfaceTrace], layout);
      }

      const tbody = document.getElementById('pos-table');
      if (!data.positions || data.positions.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" class="py-8 text-center text-gray-400">No open positions. Click GET CALL in Discover.</td></tr>';
      } else {
        tbody.innerHTML = '';
        data.positions.forEach(p => {
          const isProf = p.pnl_inr >= 0;
          const row = document.createElement('tr');
          row.className = 'hover:bg-white transition';
          row.innerHTML = `
    ...
    function buySqueeze(sym) {
      if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ action: 'SHORT_COVERING_ORDER', symbol: sym, contracts: 1 }));
      }
    }
    ...
    try:
        while True:
            ...
            if data.get("action") == "SHORT_COVERING_ORDER":
                ...
                spot = spot_prices.get(sym, 65000.0)
            ...
            async with websockets.connect(url, ping_interval=20, ping_timeout=20) as ws:
                while True:
                    ...
                    if symbol in spot_prices:
                        spot_prices[symbol] = float(data.get("c", spot_prices[symbol]))
        except Exception:
            await asyncio.sleep(3)

async def market_loop():
    while True:
        # Simulate gentle moves for XAUT (Gold)
        spot_prices["XAUTUSDT"] += np.random.normal(0, 0.2)

        pos_payloads = []
        for p in list(paper_positions):
            spot = spot_prices.get(p["symbol"], p["entry_spot"])
